...
def batchnorm_forward(x, gamma, beta, bn_param):
  ...
  if mode == 'train':
    ...
    cache = (x_nor, gamma, beta, sample_mean, sample_var, x, eps)
    running_mean = momentum * running_mean + (1 - momentum) * sample_mean
    running_var = momentum * running_var + (1 - momentum) * sample_var
...
def batchnorm_backward(dout, cache):
  ...
  x_nor, gamma, beta, sample_mean, sample_var, x, eps = cache
  N, D = dout.shape
  dx_nor = dout * gamma
  x_mu = x - sample_mean
  sample_std_inv = 1.0 / np.sqrt(sample_var + eps)  # [1,D]
  dsample_var = -0.5 * np.sum(dx_nor * x_mu, axis=0, keepdims=True) * sample_std_inv ** 3
  dsample_mean = -1.0 * np.sum(dx_nor * sample_std_inv, axis=0, keepdims=True) - 2.0 * dsample_var * np.mean(x_mu, axis=0, keepdims=True)
  dx1 = dx_nor * sample_std_inv
  dx2 = 2.0 / N * dsample_var * x_mu
  dx = dx1 + dx2 + 1.0 / N * dsample_mean
  dgamma = np.sum(dout * x_nor, axis=0, keepdims=True)
...
def max_pool_forward_naive(x, pool_param):
  ...
  for i in xrange(N):
    for j in xrange(C):
      for k in xrange(H_new):
        for l in xrange(W_new):
          out[i, j, k, l] = np.max(x[i, j, k*s:k*s+pool_h, l*s:l*s+pool_w])
...
def max_pool_backward_naive(dout, cache):
  ...
  for i in xrange(N):
    for j in xrange(C):
      for k in xrange(H_new):
        for l in xrange(W_new):
          w = x[i, j, k*s:pool_h+k*s, l*s:pool_w+l*s]
          m = np.max(w)
          dx[i, j, k*s:pool_h+k*s, l*s:pool_w+l*s] = (w == m) * dout[i, j, k, l]
